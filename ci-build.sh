...
MAX_WAIT=60
WAITED=0
until docker run --rm --network testnet martin/wait -c "${SLOWMOCKSERVER}:${SLOWMOCKSERVER_PORT}"; do
  sleep 2
  WAITED=$((WAITED+2))
  if [ "$WAITED" -ge "$MAX_WAIT" ]; then
    echo "Timeout waiting for slowmockserver to be ready"
    exit 1
  fi
  echo "Waiting for slowmockserver to be ready..."
done
...
docker run --network testnet ${INSTANCE}:${INSTANCE} --rm --entrypoint bash ngx -c "echo GET / | /usr/bin/openssl s_client -cipher 'AES256+EECDH' -tls1_2 -connect ${INSTANCE}:10443" &> /dev/null;echo "Test sslv2 not accepted...."
if docker run --network testnet ${INSTANCE}:${INSTANCE} --rm --entrypoint bash ngx -c "echo GET / | /usr/bin/openssl s_client -ssl2 -connect ${INSTANCE}:10443" &> /dev/null; then
...
docker run --network testnet ${INSTANCE}:${INSTANCE} --rm --entrypoint bash ngx -c "echo GET / | /usr/bin/openssl s_client -cipher 'DHE-RSA-AES256-SHA' -tls1_2 -connect ${INSTANCE}:10443" &> /dev/null;
...
docker run --network testnet "${MUTUAL_TLS}:${MUTUAL_TLS}" --rm martin/wait -p 10444
...
docker run --network testnet "${STANDARD_TLS}:${STANDARD_TLS}" --rm martin/wait -p 10444
